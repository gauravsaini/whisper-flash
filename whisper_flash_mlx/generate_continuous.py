...
def whisper_dflash_generate(
    ...
    if block_size is None:
        block_size = draft_model.config.block_size if draft_model is not None else 1
    mask_token_id = draft_model.mask_token_id if draft_model is not None else 0
    ...
    if block_size > 1 and draft_model is not None:
    ...
    while start < max_length:
        ...
        if current_block_size > 1:
            if is_grafted:
                ...
                draft_hidden = draft_hidden[:, :current_block_size, :]
                draft_logits = project_to_logits(target, draft_hidden[:, :-1, :])
                draft_tokens = sample(draft_logits, temperature)  # (1, current_block_size-1)
        ...
        # Get true target hidden states for this block
        if all_hidden_verify:
            true_hidden = all_hidden_verify[-1][0]  # (B, D)
        else:
            true_hidden = None
        
        for i in range(1, current_block_size):
            if i < len(block_ids_list):
                if draft_model is not None and true_hidden is not None:
                    # We verify the hidden state for draft token i (which was predicted at draft step i-1)
                    # against the target hidden state at step i-1
                ...
                # Check lexical match first (fast path), then continuous match (semantic path)
                if block_ids_list[i] == posterior_list[i - 1] or sim_val > 0.80:
                    acceptance_length += 1
                else:
                    break
            else:
        ...
        for i in range(acceptance_length + 1):
            idx = start + i
            output_list[idx] = block_ids_list[i]
        ...
        if max_prob < 0.4:
            ...
            current_block_size = max(2, block_size // 2) if draft_model is not None else 1
        else:
            current_block_size = block_size if draft_model is not None else 1
