"""P31: Lossless Speedup Stack on whisper-large-v3-turbo (MLX, local M4).

Benchmarks the following configurations on the 10 dense LibriSpeech WAV samples:
  (a) Baseline (fp16, no KV)
  (b) Lossless Stack (Q8 + KV Cache, stride-1)
  (c) Q4 Lossless Stack (Q4 + KV Cache, stride-1)
"""
...
def run_custom_config(quantize_bits: int | None, use_kv: bool, stride: int, audio_paths: list[str]) -> dict:
    cfg = ProductionConfig(
        model_path=MODEL,
        quantize=False,  # manual quantization
...
def main():
    ...
    parser.add_argument("--output", default="results/p31_lossless_stack.json")
    ...
    configs = {
        ...
        "Lossless Stack (Q8+KV)":    (8, True, 1),
        "Q4 Stack (Q4+KV)":          (4, True, 1),
    }
    ...
    print(f"\n\n{'='*80}")
    print(f"  P31 LOSSLESS STACK RESULTS — {MODEL}  (10 samples, {RUNS} runs each)")
    print(f"{'='*80}")
    ...
    out = {
        "experiment": "P31: Lossless Speedup Stack on whisper-large-v3-turbo",
        "model": MODEL,
        "n_samples": N_SAMPLES,
        "runs": RUNS,
